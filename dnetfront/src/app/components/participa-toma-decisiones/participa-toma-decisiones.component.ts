...
@Component({
  selector: 'app-participa-toma-decisiones',
  templateUrl: './participa-toma-decisiones.component.html',
  styleUrls: ['./participa-toma-decisiones.component.scss']
})
export class ParticipaTomaDecisionesComponent implements OnInit {
  form: FormGroup;
  ...
  div=document.getElementById('scrollPaDe');
  @HostListener('touchend', ['$event'])
  stop(event) {
    this.div = document.getElementById('scrollPaDe');
  ...
  ngOnInit() {
    const ac = JSON.parse(sessionStorage.getItem('participaDesiciones'));
  ...
  getDatos() {
    this.p = JSON.parse(sessionStorage.getItem('participaDesiciones'));
    this.participaDatos = this.p.participaciones || [];
    this.aclara = this.p.aclaracionesObservaciones;
    console.log("printFer", this.participaDatos);
  }

  abrirObservaciones() {
    $('body').addClass('disabled-onepage-scroll');
    $("#obs-participa-desiciones").css({ "display": "flex" });
  ...
  cerrarObservaciones() {
    ...
    $("#obs-participa-desiciones").css("display", "none");
  ...
  guardarObservaciones(){
    ...
    const participaDesi= JSON.parse(sessionStorage.getItem('participaDesiciones'));  
    participaDesi.aclaracionesObservaciones=dataAclara;
    sessionStorage.setItem('participaDesiciones', JSON.stringify(participaDesi));
    $('#obs-participa-desiciones').css('display', 'none');
    $('.menu-nav').css({ display: 'flex' });
    $('#content-btnSave').css({ display: 'flex' });
    this.acla.emit(participaDesi.aclaracionesObservaciones);
  }
  ...
  participaStatus() {
    ...
    if (this.noEmpleo == true) {
      ...
      const data = JSON.parse(sessionStorage.getItem('participaDesiciones'));
      data.ninguno = true;
      data.participaciones = null;
      sessionStorage.setItem('participaDesiciones', JSON.stringify(data));
      this.ninguno.emit(data);
      $("#abrirParticipaDesicion").css("display", "none");
      ...
      sessionStorage.setItem('participaDesiciones', JSON.stringify(data));
      this.ninguno.emit(data);
      $("#abrirParticipaDesicion").css("display", "block");
    }
  }
  ...
  eliminarDatos(a, data) {
    ...
    const spanId = `#spanEliminarPaDe${data.idPosicionVista}`;
