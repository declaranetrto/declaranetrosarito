...
public class UtileriaFunciones {
    ...
    public static boolean esDeclaracionExtemporanea(EnumTipoDeclaracion tipoDeclaracion, String sFecEncargo, String sAnioDeclaracion, Integer nivelJerarquico) {
        Calendar hoy = Calendar.getInstance();
        Calendar fecha = Calendar.getInstance();

        if (EnumTipoDeclaracion.INICIO.equals(tipoDeclaracion)){
            fecha.set(getYear(sFecEncargo), getMonth(sFecEncargo) - 1, getDay(sFecEncargo));
            if (obtieneDiferenciaEnDias(fecha, hoy) > 60) {
                return true;
            }
        }
        ...
        if (EnumTipoDeclaracion.MODIFICACION.equals(tipoDeclaracion)){
            if (hoy.get(Calendar.YEAR) != Integer.valueOf(sAnioDeclaracion) &&
            ...
            if (2020 == Integer.valueOf(sAnioDeclaracion)){//Se extiende periodo por Covid 19 hasta el 1 de Agosto --cgarias 31/05/2020
                if (4 > nivelJerarquico){//Se extiende periodo hasta el 10 de enero para decla 2020 y aquellos que nivel jerarquico sea menor a subdirector. --cgarias 26/12/2020
