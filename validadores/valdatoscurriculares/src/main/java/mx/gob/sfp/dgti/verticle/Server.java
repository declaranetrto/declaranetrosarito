...
/**
 * Clase con la definición de los paths para validar el módulo de datos curriculares de Declaranet 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo datos curriculares");
		});
	...
	/**
	 * Validador del módulo de datos curriculares
	...
	private void validarDatos(RoutingContext context) {
		...
		if(!EMPTY.equals(context.getBodyAsString())) {
			...
			try {
				DatosCurricularesDTO datos = Json.decodeValue(context.getBodyAsString(), DatosCurricularesDTO.class);
				validacion.obtenerValidaciones(datos, vertx).setHandler(modulo -> {
					if(modulo.succeeded()) {
