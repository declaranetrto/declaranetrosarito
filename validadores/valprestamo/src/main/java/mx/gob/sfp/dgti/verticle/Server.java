...
/**
 * Clase con la definición de los paths para validar el módulo de datos generales de Declaranet 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo préstamo o comodato");
		});
	...
	private void validarDatos(RoutingContext context) {
		LOGGER.info("====================== VALIDAR_PRESTAMO_COMODATO ====================== ");
		this.obtenerHeaders(context);
		if(!EMPTY.equals(context.getBodyAsString())) {
			try {
				PrestamoComodatoDTO prestamoComodatoDTO = Json.decodeValue(context.getBodyAsString(), PrestamoComodatoDTO.class);
			
				validacion.obtenerValidaciones(prestamoComodatoDTO).setHandler(modulo -> {
					if(modulo.succeeded()) {
