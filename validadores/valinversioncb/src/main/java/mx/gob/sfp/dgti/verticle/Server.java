...
/**
 * Clase con la definición de los paths para validar el módulo de inversiones, cuentas bancarias 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo inversiones, cuentas bancarias");
		});
	...
	/**
	 * Validador del módulo de inversiones, cuentas bancarias del declarante  
	...
	private void validarDatos(RoutingContext context) {
		...
		if(!EMPTY.equals(context.getBodyAsString())) {
			...
			try {
				InversionCuentaValDTO ingresos = Json.decodeValue(context.getBodyAsString(), InversionCuentaValDTO.class);
				validacion.obtenerValidaciones(ingresos, vertx).setHandler(modulo -> {
					if(modulo.succeeded()) {
