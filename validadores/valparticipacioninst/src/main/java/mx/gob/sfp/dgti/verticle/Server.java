...
/**
 * Clase con la definición de los paths para validar el módulo de participacion en instituciones 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo de participaci&oacute;n en instituciones");
		});
	...
	/**
	 * Validador del módulo de participacion en instituciones
	...
	private void validarDatos(RoutingContext context) {
		LOGGER.info("================== VALIDAR PARTICIPACION EN INSTITUCIONES ================== ");
		this.obtenerHeaders(context);

		if(!EMPTY.equals(context.getBodyAsString())) {
			
			try {
				ParticipacionInstitucionesDTO participacion = Json.decodeValue(context.getBodyAsString(), ParticipacionInstitucionesDTO.class);
				validacion.obtenerValidaciones(participacion, vertx).setHandler(modulo -> {
					if(modulo.succeeded()) {
