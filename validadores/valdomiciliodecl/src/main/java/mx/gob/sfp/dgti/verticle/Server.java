...
public class Server extends AbstractVerticle {
	...
	public void start(Promise<Void> promise) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola validador Domicilio");
		});
	...
	private void validarDomDecl(RoutingContext context) {
		...
		if(!context.getBodyAsString().equals("")) {
			try {
				...
				DomicilioDeclaranteDTO domicilioDeclaranteDto = Json.decodeValue(context.getBodyAsString(),
						DomicilioDeclaranteDTO.class);

				validadores.validar(domicilioDeclaranteDto).setHandler(modulo -> {
					if (modulo.succeeded()) {
