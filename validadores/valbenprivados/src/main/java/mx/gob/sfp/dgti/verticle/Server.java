...
public class Server extends AbstractVerticle {
	...
	public void start(Promise<Void> promise) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola validador Beneficios Privados");
		});
	...
	private void validarDomDecl(RoutingContext context) {
		...
		if(!context.getBodyAsString().equals("")) {
			try {
				...
				BeneficiosPrivadosDTO beneficiosPrivadosDto = Json.decodeValue(context.getBodyAsString(),
						BeneficiosPrivadosDTO.class);

				validacion.validar(beneficiosPrivadosDto).setHandler(modulo -> {
					if (modulo.succeeded()) {
