...
import mx.gob.sfp.dgti.dto.DatosDependienteEconDTO;
import mx.gob.sfp.dgti.dto.DatosDependientesEconDTO;
...
public class Validadores {
	 ...
	 * @since 04/11/2019
	 */
	public Future<ModuloDTO> validar(DatosDependientesEconDTO datosDependientesEcon) {
		Promise<ModuloDTO> promise = Promise.promise();

		ModuloDTO moduloDto = new ModuloDTO(EnumModulo.I_DATOS_DEPENDIENTE.getModulo());
		...
		if(datosDependientesEcon.isNinguno()) {
			propiedades.add(PropBase.crearModuloDebeSerNulo(datosDependientesEcon.getDatosDependientesEconomicos(),
					EnumCampos.DEPENDIENTES_ECONOMICOS.getCampo()));

		} else {
			propiedades.add(PropBase.crearModuloDebeSerNoNulo(datosDependientesEcon.getDatosDependientesEconomicos(),
					EnumCampos.DEPENDIENTES_ECONOMICOS.getCampo()));
			for (DatosDependienteEconDTO datos : datosDependientesEcon.getDatosDependientesEconomicos()) {
				if(datos.isVerificar()) {
					modulos.add(crearDatosDependiente(datos, datosDependientesEcon.getEncabezado()));
				} else {
					moduloDto.setIncompleto(true);
				}
			}
		}
		propiedades.add(PropBase.crearPropAclaraciones(datosDependientesEcon.getAclaracionesObservaciones()));

		new ExectValidations(client, URL_CATALOGOS).ejecutarValidacionesRx(
				new ModuloValidarDTO(EnumModulo.I_DATOS_DEPENDIENTE.getModulo(),
						modulos,
						propiedades) ,
				moduloDto)
	...
	/**
	 * Metodo para definir el submodulo de datos de dependiente
	 *
	 * @param datosDependiente
	 * @return objeto ModuloValidarDTO con las validaciones
	 *
	 * @author Pavel Alexei Martinez Regalado
	 * @since 04/11/2019
	 */
	public static ModuloValidarDTO crearDatosDependiente(DatosDependienteEconDTO datosDependiente, EncabezadoDTO encabezado){

		List<PropiedadesValidarDTO> propiedades = new ArrayList<>();
		List<ModuloValidarDTO> modulos = new ArrayList<>();

		// validar el tipoOperacion
		PropBase.crearPropTipoOperacion(datosDependiente, encabezado, propiedades);

		//Validar id
		propiedades.add(PropBase.crearId(datosDependiente.getId()));

		//Validar idPosicionVista
		propiedades.add(PropBase.crearIdPosicionVista(datosDependiente.getIdPosicionVista()));

		// validar parentescoRelacionOtro
		PropBase.crearPropCatalogoOtro(EnumCampos.RELACION_CON_DECL.getCampo(),
				EnumCatalogos.CAT_TIPO_RELACION_DEPENDIENTES.name(),
				datosDependiente.getParentescoRelacion(),
				true,
				datosDependiente.getParentescoRelacionOtro(), propiedades);

		// validar datos personales
		propiedades.add(PropBase.crearModuloDebeSerNoNulo(datosDependiente.getDatosPersonales(),
				EnumCampos.DATOS_PERSONALES.getCampo()));
		if(datosDependiente.getDatosPersonales() != null) {
			modulos.add(ValPersona.crearDatosPersonalesNoDecl(datosDependiente.getDatosPersonales()));
		}

		//Valida ciudadano extranjero
		propiedades.add(PropBase.crearModuloDebeSerNoNulo(datosDependiente.getCiudadanoExtranjero(),
				EnumCampos.CIUDADANO_EXTRANJERO.getCampo()));
		if(datosDependiente.getCiudadanoExtranjero() != null) {
			modulos.add(crearCiudadanoExtranjero(datosDependiente.getCiudadanoExtranjero()));
		}
		//Valida domicilio de dependiente
		if(datosDependiente.getHabitaDomicilioDeclarante().booleanValue()) {
			propiedades.add(PropBase.crearModuloDebeSerNulo(datosDependiente.getDomicilioDependienteEconomico(),
					EnumCampos.DOMICILIO_DEP_ECON.getCampo()));
		} else {
			propiedades.add(PropBase.crearModuloDebeSerNoNulo(datosDependiente.getDomicilioDependienteEconomico(),
					EnumCampos.DOMICILIO_DEP_ECON.getCampo()));
			modulos.add(crearDomicilioDepEcon(datosDependiente.getDomicilioDependienteEconomico()));
		}

		//Valida actividades laborales
		if (datosDependiente.isNinguno()) {
			propiedades.add(PropBase.crearModuloDebeSerNulo(datosDependiente.getActividadLaboral(),
					EnumCampos.ACTIVIDAD_LABORAL.getCampo()));
		} else {
			propiedades.add(PropBase.crearModuloDebeSerNoNulo(datosDependiente.getActividadLaboral(),
					EnumCampos.ACTIVIDAD_LABORAL.getCampo()));
			if(datosDependiente.getActividadLaboral() != null) {
				modulos.add(ValActividadLab.crearActividadLaboral(datosDependiente.getActividadLaboral(), encabezado));
			}
		}

		ModuloValidarDTO moduloValidar = new ModuloValidarDTO(
				EnumCampos.DEPENDIENTES_ECONOMICOS.getCampo() , datosDependiente.getIdPosicionVista());
	...
	/**
	 * Metodo para definir el submodulo de domicilio del dependiente economico
	 *
	 * @param
	 * @return objeto ModuloValidarDTO con las validaciones
	 *
