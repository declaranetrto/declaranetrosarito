...
public class Server extends AbstractVerticle {
	...
	public void start(Promise<Void> promise) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola validador Datos Pareja");
		});
	...
	/**
	 * Validador del módulo de dependientes economicos
	...
	private void validarDomDecl(RoutingContext context) {
		...
		if(!context.getBodyAsString().equals("")) {
			try {
				...
				DatosDependientesEconDTO datosDependientesEcon = Json.decodeValue(context.getBodyAsString(),
						DatosDependientesEconDTO.class);

				validacion.validar(datosDependientesEcon).setHandler(modulo -> {
					if (modulo.succeeded()) {
