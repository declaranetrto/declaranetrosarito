...
/**
 * Clase con la definición de los paths para validar el módulo de actividad anual anterior de Declaranet 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo actividad anual anterior");
		});
	...
	/**
	 * Validador del módulo de actividad anual anterior
	...
	private void validarDatos(RoutingContext context) {
		...
		if(!EMPTY.equals(context.getBodyAsString())) {
			try {
				ActividadAnualAnteriorDTO actividadAnualAnterior = Json.decodeValue(context.getBodyAsString(), ActividadAnualAnteriorDTO.class);
				validacion.obtenerValidaciones(actividadAnualAnterior, vertx).setHandler(modulo -> {
					if(modulo.succeeded()) {
						...
						LOGGER.error("Algo ocurrió:\n" + modulo.cause().getMessage());
