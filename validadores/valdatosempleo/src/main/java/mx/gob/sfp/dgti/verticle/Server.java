...
/**
 * Clase con la definición de los paths para validar el módulo de datos de
 * empleo en Declaranet
 * 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo v&aacute;lida empleo");
		});

		router.route().handler(BodyHandler.create());
		router.post(PATH+URL_PRIVATE_VALIDATE).handler(this::validarEmpleoCargo);
		router.route().handler(StaticHandler.create());
		this.getVertx().createHttpServer().requestHandler(router::accept)
				.listen(config().getInteger(CONFIG_PORT,
						Integer.parseInt(System.getenv(SERVER_PORT) != null ? System.getenv(SERVER_PORT) : "8188")),
						ar -> {
							if (ar.succeeded()) {
								LOGGER.info("Running");
								future.complete();
	 ...
	 */
	private void validarEmpleoCargo(RoutingContext context) {
		LOGGER.info("====================== VALIDAR_EMPLEO ====================== ");
		this.obtenerHeaders(context);
		if (!EMPTY.equals(context.getBodyAsString())) {
			try {
				DatosEmpleoCargoComisionDTO datosEmpleoCargoComisionDTO = Json.decodeValue(context.getBodyAsString(),
						DatosEmpleoCargoComisionDTO.class);
				validacion.obtenerValidaciones(datosEmpleoCargoComisionDTO).setHandler(modulo -> {
					if (modulo.succeeded()) {
						if (modulo.result().getErrores().isEmpty()  && !modulo.result().isIncompleto()) {  
							validacion.verificaEncargo(datosEmpleoCargoComisionDTO).setHandler(encargo->{
								if(encargo.succeeded()) {
									context.response().end(Json.encode(crearRespuestaCambioEncargo(encargo.result())));
								}else {
				...
				LOGGER.info("Error al validar los datos del empleo",e);
	...
	public JsonObject crearRespuestaCambioEncargo(JsonObject data){
		JsonObject respuesta = new JsonObject();
		respuesta.put("modulo", "datosEmpleoCargoComision").put("incompleto", false)
		.put("errores", new JsonArray());
		if(!data.isEmpty())
			respuesta.put("niverJerarquico", data);
		return respuesta;
	}
	/**
