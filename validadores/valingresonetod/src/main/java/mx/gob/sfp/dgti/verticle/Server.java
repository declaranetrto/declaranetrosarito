...
/**
 * Clase con la definición de los paths para validar el módulo de ingresos netos del declarante 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo ingresos netos declarante");
		});
	...
	/**
	 * Validador del módulo de ingresos netos del declarante  
	...
	private void validarDatos(RoutingContext context) {
		...
		if(!EMPTY.equals(context.getBodyAsString())) {
			...
			try {
				IngresosModulosDTO ingresosModulos = Json.decodeValue(context.getBodyAsString(), IngresosModulosDTO.class);
				validacion.obtenerValidaciones(ingresosModulos, vertx).setHandler(modulo -> {
