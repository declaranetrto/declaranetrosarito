...
public class Server extends AbstractVerticle {
	...
	public void start(Promise<Void> promise) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola validador Adeudos / Pasivos.");
		});
	...
	private void validarDomDecl(RoutingContext context) {
		...
		if(!context.getBodyAsString().equals("")) {
			try {
				...
				AdeudosPasivosDTO adeudosPasivosDto = Json.decodeValue(context.getBodyAsString(),
						AdeudosPasivosDTO.class);

				validacion.validar(adeudosPasivosDto).setHandler(modulo -> {
					if (modulo.succeeded()) {
