...
public class Propiedades {
	 ...
	 */
	public static PropiedadesValidarDTO crearNumeroCuentaContrato(String valor) {
		...
		return new PropiedadesValidarDTO(
				EnumCampos.NUMERO_CUENTA_CONTRATO.getCampo(),
				valor,
				parametros,
				true);
	}
	 ...
	 * Metodo para crear la propiedad de Monto
	 */
	public static PropiedadesValidarDTO crearSaldoInsoluto(Integer valor) {

		List<ParametrosValicacionDTO> parametros = new ArrayList<>();
		parametros.add(new ParametrosValicacionDTO(EnumValidacion.NUMERO_ENTERO));

		return new PropiedadesValidarDTO(
				EnumCampos.SALDO_INSOLUTO.getCampo(),
				valor,
				parametros,
				true);
	}
