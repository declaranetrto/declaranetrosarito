...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola validador Vehiculos");
		});
	...
	private void validarDomDecl(RoutingContext context) {
		...
		if(!context.getBodyAsString().equals("")) {
			try {
				...
				VehiculosDTO vehiculosDto = Json.decodeValue(context.getBodyAsString(),
						VehiculosDTO.class);

				validacion.validar(vehiculosDto).setHandler(modulo -> {
					if (modulo.succeeded()) {
