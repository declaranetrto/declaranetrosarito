...
public class Server extends AbstractVerticle {
	...
	public void start(Promise<Void> promise) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola validador Detalle Aviso.");
		});
	...
	private void validarDomDecl(RoutingContext context) {
		...
		if(!context.getBodyAsString().equals("")) {
			try {
				...
				DetalleAvisoDTO detalleAvisoDto = Json.decodeValue(context.getBodyAsString(),
						DetalleAvisoDTO.class);

				validacion.validar(detalleAvisoDto).setHandler(modulo -> {
					if (modulo.succeeded()) {

						if (modulo.result().getErrores().isEmpty() && modulo.result().getNiverJerarquico() == null) {
