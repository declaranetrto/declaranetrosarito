...
/**
 * Clase con la definición de los paths para validar el módulo de fideicomisos 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo fideicomisos");
		});
	...
	/**
	 * Validador del módulo de fideicomisos del declarante  
	...
	private void validarDatos(RoutingContext context) {
		...
		if(!EMPTY.equals(context.getBodyAsString())) {
			...
			try {
				ParticipacionFideicomisoDTO fideicomiso = 
						Json.decodeValue(context.getBodyAsString(), ParticipacionFideicomisoDTO.class);
				validacion.obtenerValidaciones(fideicomiso, vertx).setHandler(modulo -> {
					if(modulo.succeeded()) {
