...
/**
 * Clase con la definición de los paths para validar el módulo de participacion en empresas 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo de participaci&oacute;n en empresas");
		});
	...
	/**
	 * Validador del módulo de participacion en empresas
	...
	private void validarDatos(RoutingContext context) {
		...
		if(!EMPTY.equals(context.getBodyAsString())) {
			...
			try {
				ParticipacionEmpresasDTO participacion = Json.decodeValue(context.getBodyAsString(), ParticipacionEmpresasDTO.class);
