...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		validadores = new Validadores(vertx);
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola validador Notas");
		});
	...
	private void validarDomDecl(RoutingContext context) {
		LOGGER.info("====================== VALIDAR  NOTAS ====================== ");
		this.obtenerHeaders(context);
		if(!context.getBodyAsString().equals("")) {
			try {
				LOGGER.info(context.getBodyAsString());
				NotasAclaratoriasDTO notasAclaratoriasDTO = Json.decodeValue(context.getBodyAsString(),
						NotasAclaratoriasDTO.class);

				validadores.validar(notasAclaratoriasDTO, vertx).setHandler(modulo -> {
					if (modulo.succeeded()) {
