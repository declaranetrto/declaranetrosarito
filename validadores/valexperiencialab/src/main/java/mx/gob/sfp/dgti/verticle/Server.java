...
/**
 * Clase con la definición de los paths para validar el módulo de experiencia laboral de Declaranet 
...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo experiencia laboral");
		});
	...
	private void validarDatos(RoutingContext context) {
		...
		if(!EMPTY.equals(context.getBodyAsString())) {
			...
			try {
				ExperienciasLaboralesDTO experienciasLaborales = Json.decodeValue(context.getBodyAsString(), ExperienciasLaboralesDTO.class);
				validacion.obtenerValidaciones(experienciasLaborales, vertx).setHandler(modulo -> {
					if(modulo.succeeded()) {
