...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola validador Representacion");
		});
	...
	private void validarDomDecl(RoutingContext context) {
		...
		if(!context.getBodyAsString().equals("")) {
			try {
				...
				RepresentacionesDTO representacionesDto = Json.decodeValue(context.getBodyAsString(),
						RepresentacionesDTO.class);

				validacion.validar(representacionesDto).setHandler(modulo -> {
					if (modulo.succeeded()) {
