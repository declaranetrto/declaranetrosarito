...
public class Server extends AbstractVerticle {
	...
	public void start(Future<Void> future) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola m&oacute;dulo datos generales");
		});
	...
	private void validarDatos(RoutingContext context) {
		...
		if(!EMPTY.equals(context.getBodyAsString())) {
			try {
				DatosGeneralesDTO datosGeneralesDTO = Json.decodeValue(context.getBodyAsString(), DatosGeneralesDTO.class);
				validacion.obtenerValidaciones(datosGeneralesDTO, vertx).setHandler(modulo -> {
					if(modulo.succeeded()) {
						...
						LOGGER.error("error " + modulo.cause().getMessage());
