...
public class Server extends AbstractVerticle {
	...
	public void start(Promise<Void> promise) throws Exception {
		...
		router.route(PATH).handler(routingContext -> {
			...
			response.putHeader(HEADER_CONTENT_TYPE, TEXT_HTML).end("Hola validador bienes inmuebles");
		});
	...
	private void validarDomDecl(RoutingContext context) {
		...
		if(!context.getBodyAsString().equals("")) {
			try {
				...
				BienesInmueblesDTO bienesInmueblesDto = Json.decodeValue(context.getBodyAsString(),
						BienesInmueblesDTO.class);

				validacion.validar(bienesInmueblesDto).setHandler(modulo -> {
					if (modulo.succeeded()) {
