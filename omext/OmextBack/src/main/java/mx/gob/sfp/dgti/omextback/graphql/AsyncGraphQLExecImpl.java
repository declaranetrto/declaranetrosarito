    	...
    	try {
            graphQLProvider.graphQL()
                    ...
                    .handle((executionResult, exception) -> {
                        ...
                        if (!errors.isEmpty()) {
                            if(executionResult.getErrors().get(0) instanceof ExceptionWhileDataFetching) {
                                ExceptionWhileDataFetching e = (ExceptionWhileDataFetching) executionResult.getErrors().get(0);

                                future.fail(e.getException());
                            } else {
                                future.fail(new SFPException(EnumError.ERROR_GRAPHQL, errors.toString()));
                            }
            ...
            LOGGER.info("Error en AsyncGraphQLExecImpl" + e);
        	future.fail(e);
        }
    	return future;
    }
