...
export class PerfilesComponent implements OnInit {
  ...
  async setProfile() {
    ...
    const entesId = [];
    this.globals.entes.forEach(ente => {
      entesId.push({id: ente.id});
    });
    perfil.entes = entesId;
    this.idpService.seleccionarPerfil(perfil).subscribe(
      (dataSeleccionarPerfil) => {
        ...
        this.globals.setValueSession('entes', dataSeleccionarPerfil.body.perfilBody.entes);
